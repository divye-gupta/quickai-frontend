...
function Markup() {
        ...
        if (response.data.length > 0) {
          ...
          array.push({
            ...
            time:
              ...
              "" +
              date.toLocaleTimeString().substring(9, 12),
          });
          setCreated(array);
          setShowData(true);
        }
    ...
    Axios.get("http://localhost:8000/getmarkup")
      .then((response) => {
        if (
          ...
          let selectOptionObject;

          if (selectedProduct?.label === "Air") {
            selectOptionObject = {
          ...
          } else if (selectedProduct?.label === "Hotel") {
            selectOptionObject = {
              ProductType: selectedProduct.label,
              Amount: amount,
              AmountType: "Fixed",
          ...
          Axios.post("http://localhost:8000/addmarkup", selectOptionObject)
  ...
  const deleteMarkup = () => {
    const del = async () => {
      Axios.get("http://localhost:8000/deletemarkup").then((response) => {
        setShowData(false);
  ...
  return (
    <div>
      ...
      {/* Markup Table*/}
      {showData && markupData.ProductType === "Air" ? (
            ...
            <TableBody>
              <StyledTableRow>
                <StyledTableCell component="th" scope="row">
                  {created[0]?.month} {created[0]?.date},{created[0]?.year}{" "}
                  {created[0]?.time}
                </StyledTableCell>
                <StyledTableCell align="center">
                  {markupData.ProductType}
                </StyledTableCell>
                <StyledTableCell align="center">
                ...
                <StyledTableCell align="center">
                  <Button onClick={() => editMarkup(markupData._id)}>
                    <Edit />
                  </Button>
                </StyledTableCell>
                <StyledTableCell align="center">
                  <Button onClick={() => deleteMarkup()}>
                    <Delete />
                  </Button>
                </StyledTableCell>
              </StyledTableRow>
      ...
      {/* hotel markup table */}
      {showData && markupData.ProductType === "Hotel" ? (
        <TableContainer component={Paper}>
          <Table className={classes.table} aria-label="customized table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell align="right">Pax Types</StyledTableCell>
                <StyledTableCell align="right">Amount Type</StyledTableCell>
                <StyledTableCell align="right">Value (in %)</StyledTableCell>
                <StyledTableCell align="center">Action</StyledTableCell>
                <StyledTableCell align="center">Delete</StyledTableCell>
              </TableRow>
            </TableHead>
